<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">Editer le type : {{ $type->name }}</h2>
    </x-slot>

    <div class="py-8">
        <div class="max-w-2xl mx-auto sm:px-6 lg:px-8">
            <form method="POST" action="{{ route('admin.card-types.update', $type) }}" class="bg-white rounded-xl shadow-sm p-6 space-y-5">
                @csrf @method('PUT')
